...
def add_hydrogen(n: pypsa.Network, costs: pd.DataFrame) -> None:
    ...
    for h2_tech in h2_techs:
        ...
        n.add("Link", spatial.nodes + " " + h2_tech, **link_kwargs)
...
def define_spatial(nodes, options):
    ...
    if options["biomass"]["biomass_transport"]:
...
def add_biomass(n, costs):
    ...
    biomass_pot = (
        snakemake.params.sector_options["biomass"]["user_defined_potentials"]["solid_biomass_potential"] * 1e6
    )  # MWh
    biogas_pot = snakemake.params.sector_options["biomass"]["user_defined_potentials"]["biogas_potential"] * 1e6  # MWh
        ...
        if countries_not_in_index:
            logger.info(
                "No transport values found for {0}, using default value of {1}".format(
                    ...
                    snakemake.params.sector_options["biomass"]["biomass_transport_default_cost"],
                )
            )

        bus0_costs = biomass_transport.bus0.apply(
            lambda x: transport_costs.get(
                x[:2], snakemake.params.sector_options["biomass"]["biomass_transport_default_cost"]
            )
        )
