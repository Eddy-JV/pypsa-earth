...
def add_h2_network_cap(n, cap):
    ...
    # If nothing is extendable, there is no Link-p_nom investment var → nothing to cap
    if not h2_network.p_nom_extendable.any():
        return
...
def hydrogen_temporal_constraint(n, n_ref, time_period):
    ...
    res = (weightings_gen * p_gen_var).sum(dim="name")
    ...
    if not res_stor_index.empty:
        ...
        store = (weightings_stor * p_dispatch_var).sum(dim="name")
